#!/usr/bin/env bash
# Populate vendor/store/ with symlinks to fabric-store's C sources.
#
...
# When repo sync populates vendor/store/ automatically, this script becomes
# unnecessary; until then, run it after checkout.
set -euo pipefail
...
mkdir -p "${here}/vendor/store"
for f in fdb_vfs.c fdb_keys.h; do
	if [[ ! -f "${store}/${f}" ]]; then
		echo "missing source: ${store}/${f}" >&2
		exit 1
	fi
	ln -sfn "${store}/${f}" "${here}/vendor/store/${f}"
	echo "linked vendor/store/${f} -> ${store}/${f}"
done
